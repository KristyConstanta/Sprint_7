...
class TestLoginCourier:
    ...
    def test_get_courier_id_positive(self):
    ...
    def test_get_courier_id_negative(self):
        login_pass = register_new_courier_and_return_login_password()
        payload = {
            "login": login_pass[0],
    ...
    @allure.title('Авторизация курьера не пройдена при при отсутствии поля пароль')
    @allure.description(
    ...
    @allure.title('Авторизация курьера не пройдена при при отсутствии поля логин')
    @allure.description(
